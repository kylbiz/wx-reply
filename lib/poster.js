...
Poster.prototype.RemoveUserImg = function(options, callback) {
	...
	if(!options
		...
		for(var i = 0; i < length; i++) {
			fs.unlink(imgLists[i], function(err) {
				if(err) {
					log("RemoveUserImg: error remove file " + imgLists[i], err);
					// callback("RemoveUserImg: error remove file " + imgLists[i]);
				} else {
					log("RemoveUserImg: succeed remove file " + imgLists[i]);
				}
			})
		}
	}
