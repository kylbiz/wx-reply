    	...
    	if(!result.hasOwnProperty('xml')) {
			...
			fillWord(options, function(err, image) {
				if(err) {
					...
					handlePhoto(myoptions, function(err, image2) {
						log(arguments)
						log("Then upload photo")
						api.uploadMedia(image2, type, function(err, result) {
							if(err) {
