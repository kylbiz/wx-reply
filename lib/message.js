...
MessageServer.prototype.getMessage = function(req, res, next) {
    ...
    if(err) {
    	...
    	if(!result.hasOwnProperty('xml')) {
	    	...
	    	if(MsgType === "event" && xml.hasOwnProperty("Event") && xml.Event[0] === "CLICK" && xml.EventKey[0] === "generate_card") {
	    		generate_flag = true;
	    	} else if(MsgType === "text" && xml.hasOwnProperty("Content") && (xml.Content[0] === "开业啦" || xml.Content[0] === "刘大哥")){
	    		generate_flag = true;
	    	}
		    			...
		    			if(!user || !user.hasOwnProperty("nickname")) {
		    				...
		    				poster.getWxUserImg(wxImgOptions, function(err, imgInfo) {
		    					if(err) {
		    						...
		    						// 随机选择背景图片
		    						var canvasNum = Math.floor(Math.random() * 7) + 1;
